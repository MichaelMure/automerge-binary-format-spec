...
An actor applies a linear sequence of <<Operation,operations>> to a <<Document,document>>
...
| message | 53 | 3 | <<String Column>> | The (optional) commit message for each change
...
[cols="2,1,1,2,1,1,4"]
|===
| Field | Spec value | ID | Type | In a document chunk | In a change chunk | Description

| object actor ID | 1 | 0 | <<Actor Column>> | yes | yes | actor index of object ID each operation targets
| object counter | 2 | 0 | <<uLEB Column>> | yes | yes | counter of the object ID each operation targets
| key actor ID | 17 | 1 | <<Actor Column>> | yes | yes | actor of the operation ID of the key of each operation
| key counter | 19 | 1 | <<Delta Column>> | yes | yes | counter of the operation ID of the key of each operation
| key string | 21 | 1 | <<String Column>> | yes | yes | The string key each operation targets
| actor ID | 33 | 2 | <<Actor Column>> | yes | no | The actor of each operations ID
| counter | 35 | 2 | <<Delta Column>> | yes | no | The counter of each operations ID
| insert | 52 | 3 | <<Boolean Column>> | yes | yes | Whether or not this is an insert operation
| action | 66 | 4 | <<uLEB Column>> | yes | yes | The <<Action>> of each operation
| value metadata | 86 | 5 | <<Value Metadata Column>> | yes | yes | The metadata for the value of this operation
| value | 87 | 5 | <<Value Column>> | yes | yes | The value of this operation
| predecessor group | 112 | 7 | <<Group Column>> | no | yes | The group for the predecessors of this operation (only in <<change-chunks,change chunks>>)
| predecessor actor IDs | 113 | 7 | Grouped <<Actor Column>> | no | yes | The actor ID of each predecessor's operation ID
| predecessor counters | 115 | 7 | Grouped <<Delta Column>> | no | yes | The counter of each predecessor's operation ID
| successor group | 128 | 8 | <<Group Column>> | yes | no | The group for the successors of this operation (only in <<Document Chunk,document chunks>>)
| successor actor IDs | 129 | 8 | Grouped <<Actor Column>> | yes | no | The actor ID of each successor's operation ID
| successor counters | 131 | 8 | Grouped <<Delta Column>> | yes | no | The counter of each successor's operation ID
| mark expand | 148 | 9 | <<Boolean Column>> | yes | yes | How the mark expands on the text
| mark name | 165 | 10 | <<String Column>> | yes | yes | The name of the formatting mark
|===

WARNING: The javascript implementation includes a `child` column, is this
required?

Here is how those fields are to be interpreted:

* Object:
  - If both object actor ID and object counter are null, object is the root object (actor ID=0, counter=0)
  - If both object actor ID and object counter are not null, use those values to construct object
  - Other cases are invalid and implementation MUST abort
* Key:
...
// below would deserve a better explanation
  - If key actor ID is null and key counter equals 0, key is the pair (lookup_actor(0), key counter)
  - If key string is null and key counter is null, implementations MUST abort
* ID:
  - If both actor ID and counter are not null, ID is the pair (lookup_actor(actor ID), counter)
  - If either actor ID or counter are null, implementations MUST abort
* Insert:
  - The non-nullable value is used directly
* Action:
  - If a null value is read, implementations MUST abort, otherwise use that value.
...
// TODO: below
| Predecessors | Operation IDs | Past operations that ??? (if any)
| Successors | Operation IDs | Future operations that affect the object created by this operation (if any)
// TODO: rich text columns
|===
...
encoding>> to compress a list of 64-bit <<LEB>>s.
...
you would end up with `0x7e 0x01 0x61 0x00 0x00 0x01 0x02 0x03 0x62 0x6f 0x6f`.
...
list of 64-bit <<uLEB>>s that defines the type and length of each value in the
value column.
...
| 5 | IEEE754 float | 64-bit little-endian IEEE754 float in value column (length = 8)
